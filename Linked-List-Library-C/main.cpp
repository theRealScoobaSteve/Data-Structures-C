...
int main() {
    try {
        ...
        cout << endl << copyConstructorTest(test);
        testReverse(test);
        arithmeticTest();
        testIndex();
    }
...
void testIndex () {
    ...
    if( test[0] != 'T')
        throw my_string("The first letter is not correct!----error----");

    if( test.indexOf('T') != 0 )
        throw my_string("Index returned is not correct!-----error----");
...
void arithmeticTest( ) {
    ...
    if( temp3 == "HelloWorld")
        ...
        throw my_string("These values are not the same!---error----\n");
